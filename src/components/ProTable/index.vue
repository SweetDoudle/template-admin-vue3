...
const props = defineProps({
  ...
  searchCol: {
    // 表格搜索项 每列占比配置 ==> 非必传默认为 { xs: 1, sm: 2, md: 2, lg: 3, xl: 4 }
    type: Object,
    default: () => {
      return {
        xs: 1,
...
onMounted(() => {
  ...
  props.data.length > 0 && (pageable.value.total = props.data.length);
});
...
const flatColumns = computed(() => {
  return flatColumnsFunc(tableColumns);
});
...
// 定义 emit 事件
const emit = defineEmits(["search", "reset", "dargSort"]);
